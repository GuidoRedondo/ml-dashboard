    ...
    for (let i = 0; i < allItemIds.length; i += PARALLEL) {
      ...
      await Promise.all(batch.map(async itemId => {
        try {
          ...
          itemsMap[b.id]   = { mla_id: b.id, title: b.title, sku, price, original_price: precioLista, stock: b.available_quantity, listing_type_id: b.listing_type_id, category_id: b.category_id, logistic_type: b.shipping?.logistic_type || 'cross_docking' };
        } catch(e) {}
      }));
    }
